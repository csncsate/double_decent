...
def train_with_batches(model, data, eval_loss, n_iterations, device, batch_size=16):
    ...
    #     torch.manual_seed(i + int(training_ratio * 100000) + 100)
    #     y = graph.y.detach().clone().cpu()
    #     samples_per_class = torch.ones(num_classes, dtype=torch.int)
    #     for i in range(num_classes):
    #         samples_per_class[i] = torch.sum(y == i)
    #     samples_per_class=(samples_per_class * training_ratio).int()
    #     rangemask = torch.randperm(graph.num_nodes)
    #     for i in range(graph.num_nodes):
    #         ii = rangemask[i]
    #         yi = y[ii]
    #         if samples_per_class[yi] > 0:
    #             train_mask[ii] = True
    #             test_mask[ii] = False
    #             samples_per_class[yi] = samples_per_class[yi] - 1
    #         if torch.sum(samples_per_class) == 0:
    #             break
    # sample_count_per_class = int(graph.num_nodes * training_ratio / num_classes)
    # for c in range(num_classes):
    #     idx = idx_per_class[c]

    # train_mask[idx[:int(graph.num_nodes * training_ratio)]] = True
